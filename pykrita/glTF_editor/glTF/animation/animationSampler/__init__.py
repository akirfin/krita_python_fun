from enum import Enum

from glTF_editor.common.utils_py import \
        UnicodeType

from glTF_editor.common.data_serializer import \
        serializer


class AnimationSampler(object):
    """
    ...
    """

    def __init__(self,
                input=None,
                output=None,
                interpolation=None,
                extensions=None,
                extras=None):

        self._input = 0
        self._output = 0
        self._interpolation = Interpolation.LINEAR
        self._extensions = None
        self._extras = None

        if input is not None:
            self.input = input
        if output is not None:
            self.output = output
        if interpolation is not None:
            self.interpolation = interpolation
        if extensions is not None:
            self.extensions = extensions
        if extras is not None:
            self.extras = extras


    @classmethod
    def cast(cls, obj):
        if isinstance(obj, cls):
            return cls(
                    input=obj.input,
                    output=obj.output,
                    interpolation=obj.interpolation,
                    extensions=obj.extensions,
                    extras=obj.extras)
        elif isinstance(obj, Mapping):
            return cls(
                    input=obj.get("input"),
                    output=obj.get("output"),
                    interpolation=obj.get("interpolation"),
                    extensions=obj.get("extensions"),
                    extras=obj.get("extras"))
        elif isinstance(obj, UnicodeType):
            unserialized = serializer.loads(obj)
            if not isinstance(unserialized, UnicodeType):
                return cls.cast(unserialized)
        elif isinstance(obj, Iterable):
            it = iter(obj)
            return cls(
                    input=next(it),
                    output=next(it),
                    interpolation=next(it),
                    extensions=next(it),
                    extras=next(it))
        raise RuntimeError("Unable to Cast {obj!r} to {cls} instance.".format(**locals()))


    def getInput(self):
        if self._input is None:
            return None
        else:
            return iter(self._input)
    def setInput(self, newInput):
        if newInput is None:
            self._input = None
        else:
            self._input = [UnicodeType(n) for n in newInput]
    input = property(getInput, setInput)


    def getOutput(self):
        if self._output is None:
            return None
        else:
            return iter(self._output)
    def setOutput(self, newOutput):
        if newOutput is None:
            self._output = None
        else:
            self._output = [UnicodeType(n) for n in newOutput]
    output = property(getOutput, setOutput)


    def getInterpolation(self):
        if self._interpolation is None:
            return None
        else:
            return iter(self._interpolation)
    def setInterpolation(self, newInterpolation):
        if newInterpolation is None:
            self._interpolation = None
        else:
            self._interpolation = [UnicodeType(n) for n in newInterpolation]
    interpolation = property(getInterpolation, setInterpolation)


    def getExtensions(self):
        if self._extensions is None:
            return None
        else:
            return iter(self._extensions)
    def setExtensions(self, newExtensions):
        if newExtensions is None:
            self._extensions = None
        else:
            self._extensions = [UnicodeType(n) for n in newExtensions]
    extensions = property(getExtensions, setExtensions)


    def getExtras(self):
        if self._extras is None:
            return None
        else:
            return iter(self._extras)
    def setExtras(self, newExtras):
        if newExtras is None:
            self._extras = None
        else:
            self._extras = [UnicodeType(n) for n in newExtras]
    extras = property(getExtras, setExtras)


    def __str__(self):
        return serializer.dumps(self, type_hints=False)


    def __repr__(self):
        cls = type(self)
        return ("{cls.__name__}("
                "input={self.input!r}, "
                "output={self.output!r}, "
                "interpolation={self.interpolation!r}, "
                "extensions={self.extensions!r}, "
                "extras={self.extras!r}")").format(**locals())


def to_dict(obj):
    result = oDict()
    def add_valid(attr_name):
        value = getattr(obj, attr_name, None)
        if value is not None:
            result[attr_name] = value.value if isinstance(value, Enum) else value
    add_valid("input")
    add_valid("output")
    add_valid("interpolation")
    add_valid("extensions")
    add_valid("extras")
    return result


serializer.register(
        data_cls=AnimationSampler,
        from_dict=lambda dct: AnimationSampler.cast(dct),
        to_dict=to_dict)
